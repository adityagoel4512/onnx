...
class TestShapeInference(TestShapeInferenceHelper):
    ...
    @unittest.skipUnless(ONNX_ML, "ONNX_ML required to test ai.onnx.ml operators")
    def test_tree_ensemble_regressor_opset5(self) -> None:
        interior_nodes = 5
        leaves = 9
        tree = make_node(
            "TreeEnsembleRegressor",
            ...
            leaf_weights=make_tensor(
                "leaf_weights", TensorProto.FLOAT, (leaves,), [1] * leaves
            ),
            tree_roots=[0],
        )

        graph = self._make_graph(
            [("x", TensorProto.FLOAT, ("Batch Size", "Features"))],
            [tree],
            [],
        )
        ...
        self._assert_inferred(
            graph,
            [make_tensor_value_info("y", TensorProto.FLOAT, ("Batch Size", 5))],
            opset_imports=[
    ...
    @unittest.skipUnless(ONNX_ML, "ONNX_ML required to test ai.onnx.ml operators")
    def test_tree_ensemble_regressor_opset5_fails_if_invalid_attributes(
        self,
        nodes_truenodeids,
        leaf_weights,
        nodes_splits,
