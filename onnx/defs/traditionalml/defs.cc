...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .Attr(
            "default_tensor",
            "A default tensor.",
            "{\"_Unused\"} if values_* has string type, {-1} if values_* has integral type, and {-0.f} if values_* has "
            "float type.",
            AttributeProto::TENSOR)
        .TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
...
// Switch some ints to storing attr in tensorproto with data type uint8
...
ONNX_ML_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint("T", {"tensor(float)", "tensor(double)"}, "The input type must be a tensor of a numeric type.")
        .Attr("nodes_featureids", "Feature id for each node.", AttributeProto::INTS, true)
        .Attr(
            "nodes_splits",
            "Thresholds to do the splitting on for each node with mode that is not 'SET_MEMBER'.",
            AttributeProto::TENSOR,
            true)
        .Attr(
            "nodes_hitrates",
        ...
        .Attr(
            ...
            "order that the 'SET_MEMBER' mode appears in `node_modes`, delimited by `NaN`s. Will have the same number "
            "of sets of values as nodes with mode 'SET_MEMBER'. This may be ommitted if the node doesn't contain any "
            "'SET_MEMBER' nodes.",
            AttributeProto::TENSOR,
            OPTIONAL_VALUE)
        .Attr(
            ...
            AttributeProto::INTS,
            true)
        .Attr("leaf_weights", "The weight for each target", AttributeProto::TENSOR, true)
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
          ...
          AssertAttributeProtoType(ctx.getAttribute("nodes_modes"), expected_length, TensorProto_DataType_INT64, true);
          AssertAttributeProtoType(
